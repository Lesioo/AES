...
void aes256_init(aes256_context *ctx, uint8_t *k)
{
	...
	for (i = 8; --i;) {
		...
		//printf("\n i=%d  ", i);
		//for (j = 0; j < 32; j++) printf("%02x ", ctx->deckey[j]);
	}
} /* aes256_init */
...
void aes256_encrypt_ecb(aes256_context *ctx, uint8_t *buf)
{
	...
	for (i = 0; i < 16; i++) printf("%02x ", buf[i]);
	printf("\n");
	for (i = 0; i < 32; i++) printf("%02x ", ctx->key[i]);
	printf("\n");
    aes_subBytes(buf);
    aes_shiftRows(buf);
...
__declspec(dllexport) char * encrypt(char* table, char* keyIN)
{
	aes256_context ctx;
	uint8_t key[32];
	uint8_t buf[16];
	...
	for (i = 0; i < sizeof(buf); i++) buf[i] = table[i];
	for (i = 0; i < sizeof(key); i++) key[i] = keyIN[i];
	for (i = 0; i < sizeof(key); i++) printf("%02x ", key[i]);
	aes256_init(&ctx, key);
	printf("\nINIT:\n");
	for (i = 0; i < sizeof(key); i++) printf("%02x ", ctx.deckey[i]);
	printf("\n");
	aes256_encrypt_ecb(&ctx, buf);

	for (i = 0; i < sizeof(buf); i++) ret[i] = buf[i];
	printf("\nZaszyfrowane\n");
...
__declspec(dllexport) char * decrypt(char* table, char* keyIN)
{
	aes256_context ctx;
	uint8_t key[32];
	uint8_t buf[16];
